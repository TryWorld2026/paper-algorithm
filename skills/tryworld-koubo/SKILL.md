...
- 结构：开场钩子（0-15 秒）→ 3-5 层讲解 → 结论/收尾 + 固定签名（签名语以 paper 当前主题文件的 sign_off 为准；默认主题为"我是试界TryWorld，持续把 AI 讲清楚，让每个普通人都看得懂、用得上。觉得这条视频有用，点个赞，也欢迎关注，谢谢大家，下期见。"）。
...
- 音色：默认流程不询问，统一云希；用户主动要求换声音（女声/男声/换个音色等）时，把该偏好带入 paper 流程，由 paper 按「音色选择（静默默认，按需试听）」规则用稿子开头合成试听供用户挑选。
